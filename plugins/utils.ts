...
import { chalk, glob, winPath } from '@umijs/max/plugin-utils';
import {
  Loader,
  TransformResult,
...
export class ModelUtils {
  ...
  getAllModels(opts: { sort?: object; extraModels: string[] }) {
    console.log('开始获取所有模型文件...');
    console.log('extraModels:', opts.extraModels);
  ...
  isModelValid(opts: { content: string; file: string }) {
    ...
    try {
      ...
      if (e.errors?.length) {
        console.error(e.errors, { path: file, content });
        delete e.errors;
      }
      throw e;
    }
