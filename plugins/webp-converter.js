...
const glob = require('glob'); // 文件匹配模块，用于查找符合条件的文件
const sharp = require('sharp'); // 图片处理库，用于转换图片格式
...
module.exports = (api) => {
  ...
  const config = api.userConfig.webpConverter || {};
  ...
  api.onBuildComplete(({ err }) => {
    ...
    const imageFiles = glob.sync(
      path.join(distDir, 'static/**/*.{jpg,jpeg,png}'),
    );
    ...
    const promises = imageFiles.map(async (imagePath) => {
      try {
        totalFiles++; // 处理文件总数加1
        ...
        if (webpSize < originalSize || !onlySmallerFiles) {
          // 构建 WebP 文件路径，与原文件同目录，文件名添加 .webp 后缀
 ...
 * @returns {string} 格式化后的文件大小（如 "1.25 KB"）
 */
