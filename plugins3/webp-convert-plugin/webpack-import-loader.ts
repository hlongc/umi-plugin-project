...
export default function loader(
  ...
  if (shouldOnlyRemoveWebpMark) {
    ...
    if (hasModifications) {
      console.log(
        `[webp-convert-plugin] 开发模式: 在 ${resourcePath} 中移除了 !webp 标记`,
      );
    }
  ...
  if (isFileDisabled) {
    console.log(
      `[webp-convert-plugin] 文件 ${resourcePath} 已通过顶部注释禁用 WebP 转换`,
    );
  }
  ...
  const processImports = async (): Promise<void> => {
    for (const pattern of importPatterns) {
      ...
      while ((match = pattern.regex.exec(source)) !== null) {
        ...
        try {
          ...
          // 检查是否已经处理过该图片
          if (context.existProcessedImage(imagePath)) {
          ...
          if (fs.existsSync(imagePath) && !importInfo.skipWebpConversion) {
            // 将图片路径添加到已处理缓存中
            context.addProcessedImage(imagePath);
            ...
            if (webpSize < originalSize || !onlySmallerFiles) {
              ...
              // 将WebP文件路径添加到临时文件数组中
              context.addTmpWebpFile(webpPath);
    ...
    if (imports.length > 0) {
      console.log(
        `[webp-convert-plugin] 在 ${resourcePath} 中找到 ${imports.length} 个图片导入:`,
      );
      imports.forEach((imp) => {
        console.log(
